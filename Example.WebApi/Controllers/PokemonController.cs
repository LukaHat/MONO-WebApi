...
namespace Example.WebApi.Controllers
{
    public class PokemonController : ApiController
    {
        ...
        [HttpGet]
        public async Task<HttpResponseMessage> GetAllPokemons([FromUri]string nameQuery="", string typeQuery = "", string secondTypeQuery = "" , int pageNum = 1, int pageSize = 10, string sortOrder = "ASC", string sortBy = "TrainerId")
        {
            Task<List<PokemonRead>> pokemons = pokemonService.GetAllPokemonsAsync(nameQuery, typeQuery, secondTypeQuery, pageNum = 1, pageSize = 10, sortOrder = "ASC",  sortBy = "Id");
            await pokemons;
            if (pokemons == null)
            {
